...
export const getAllComment = TryCatch(async (req:Request,res:Response) => {
  const {id} = req.params;
...
export const saveBlogs = TryCatch(async(req:AuthenticatedRequest,res:Response) => {
  const {blogid} = req.params;
  const userid = req.user?._id;

  if(!blogid || !userid){
    res.status(400).json({
      message: "Blogid or Userid missing"
    });
    return;
  }
...
export const getSavedBlog = TryCatch(async(req:AuthenticatedRequest,res:Response) => {
  ...
  const blogs = await sql `SELECT * FROM savedblogs WHERE userid = ${blogid}`;
